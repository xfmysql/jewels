...
import com.its.jewels.entity.BackgroundCell;
import com.its.jewels.entity.BorderSprite;
import com.its.jewels.entity.JewelSprite;
...
public class Jewels extends BaseGameActivity implements IOnSceneTouchListener, IConstants {
	...
	private int mTime = 0;//mTime秒后还不做任何操作就自动提示
	...
	public Engine onLoadEngine() {
		...
		return new LimitedFPSEngine(new EngineOptions(true, ScreenOrientation.PORTRAIT,
				new RatioResolutionPolicy(CAMERA_WIDTH, CAMERA_HEIGHT), this.mCamera).setNeedsSound(true),50);//需要播放音效
	}
	...
	public boolean onSceneTouchEvent(final Scene pScene, final TouchEvent pSceneTouchEvent) {
		...
		if(pSceneTouchEvent.getX() > 0 && pSceneTouchEvent.getX() < CAMERA_WIDTH
			...
			if(pSceneTouchEvent.getAction() == MotionEvent.ACTION_UP) {
				Log.e("debug","场景按上：("+pSceneTouchEvent.getX() +","+pSceneTouchEvent.getY()+")="+CELL_WIDTH+"="+this.mCurRow );
				...
				Log.e("debug","场景按下：("+pSceneTouchEvent.getX() +","+pSceneTouchEvent.getY()+")="+CELL_WIDTH+"="+this.mCurRow );
		...
		public void handleMessage(Message msg) {
			switch (msg.what) {
				...
				case 4:
					...
					break;
				case 5://自定义消息
					showLongMessage("我的提示："+msg.obj.toString());
					break;
				default:
					break;
			}
			...
			public void onUpdate(float pSecondsElapsed) {
				...
				if(Jewels.this.mGameRunning){
					switch (STATE) {
						...
						case CHECK:
							checkMapDead(); //可消除检测
							removeHorizontal(); //水平消去，设置消除的钻石为缩放
							removeVrtical();    //垂直消去
							changeState();//只要有缩放状态的钻石，设置状态为fall
							break;
						case FALL:
							refreshScale(); //消去动画
							fillEmpty();    //填充空缺
	...
	private void initJewels(){
		...
		for(int i = 0; i < CELLS_HORIZONTAL; i++){
			for(int j = 0; j < CELLS_VERTICAL; j++){
				String key = getKey(i, j);//00,01,02,03
				...
				this.mMainScene.getLayer(LAYER_JEWELS).addEntity(this.mHashMap.get(key).getJewel());
				Sprite jewels = this.mHashMap.get(key).getJewel();
				mMainScene.registerTouchArea(jewels);///
			}
		}
	...
	private void doTips(){
		...
		if(mDeadArrList.size() > 0){
			int randindex = MathUtils.random(0, mDeadArrList.size()-1);
			String key = mDeadArrList.get(randindex);
			...
			//Log.e("debug", "==========提醒检测可消除"+mDeadArrList.size());
			//checkMapDead();//检测可消除的钻石，放入mDeadArrList
			//Log.e("debug", "==========提醒检测结束"+mDeadArrList.size());
		}
	}
	...
	/**
	 * 填充空缺位置，上一行的移动下来，补充mHashMap里面的钻石，最后钻石下落
	 */
		...
		for(int i = 0; i < CELLS_HORIZONTAL; i++)
		{
			for(int j = 0; j < CELLS_VERTICAL; j++)
			{
				if(mHashMap.get(getKey(j, i)).getState() == STATE_DEAD)
				{
					int p = i;
					while((p-1) >= 0  &&
							mHashMap.get(getKey(j, p-1)).getState() != STATE_DEAD){//mHashMap里面移动位置
	...
	/**
	 * 检测所有钻石，放入mDeadArrList数组
	 */
	private synchronized void checkMapDead(){
			...
			if(this.mDeadArrList.size() == 0){
				...
				while(i < CELLS_HORIZONTAL){
					...
					while(j < CELLS_HORIZONTAL){
						if(checkDead(mHashMap.get(getKey(j, i))) == true){//用这个改造替换isSwapFall()
				...
				//	STATE= DEAD;
				//}
				Log.e("debug", "10秒钟提示=======================可消除检测"+mDeadArrList.size());
			}
		}
	}

	/**
	 * 检测单个，交换试探检测,true可以交换
	 */
	private boolean checkDead(final JewelSprite sprite){
		...
		if((col - 1) >= 0){
			...
			for(v = 1; col-1-v >= 0
				...
				v++);
			//Log.e("debug","向上检查：("+row+","+col+")，v上="+v);
			if(v >= 3){
				flag = true;
				//Log.e("debug","可以消除：("+row+","+col+")");
			}
		...
		if((col + 1) < CELLS_HORIZONTAL){
			...
			for(v1 = 1; col+1+v1 < CELLS_HORIZONTAL
				...
				v1++);
			//Log.e("debug","向下检查：("+row+","+col+")，v下="+v1);
			if(v1 >= 3){
				flag = true;
			}
			mHashMap.put(getKey(row, col+1), mHashMap.get(getKey(row, col)));
		...
		if((row - 1) >= 0){
			...
			for(v2 = 1; row-1-v2 >= 0
				...
				v2++);
			//Log.e("debug","向做检查：("+row+","+col+")，v左="+v2);
			if(v2 >= 3){
				flag = true;
			}
		...
		if((row + 1) < CELLS_VERTICAL){
			...
			for(v3 = 1; row+1+v3 < CELLS_VERTICAL
				...
				v3++);
			//Log.e("debug","向右检查：("+row+","+col+")，v右="+v3);
			if(v3 >= 3){
				flag = true;
			}
			mHashMap.put(getKey(row+1, col), mHashMap.get(getKey(row, col)));
	...
	/**
	 * 在check状态，是在未操作下落过程中水平消去，设置成缩放
	 */
	private void removeHorizontal(){
		int k = 0;
		for(int i = 0; i < CELLS_VERTICAL; i++)//垂直
		{
			for(int j = 0; j < CELLS_HORIZONTAL - 2; j++)
			{
				if(mHashMap.get(getKey(j, i)).getState() == STATE_NORMAL){
					for(k = 1;
						j+k < CELLS_HORIZONTAL &&  mHashMap.get(getKey(j, i)).getStyle() == mHashMap.get(getKey(j+k, i)).getStyle() &&  mHashMap.get(getKey(j, i)).getState() == mHashMap.get(getKey(j+k, i)).getState();
						k++);//判断相同的

					if(k >= 3)
					{
						Log.d("debug","水平消除的坐标和长度"+i+"="+j+"="+k);
						this.addBonus();
						this.addScore(k);
	...
	private void addBonus(){
		...
		if(this.mBonus.getWidth() > 299){//299
		...
		Dialog dialog = new AlertDialog.Builder(this)
				...
				.setPositiveButton("是", new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int whichButton) {
						//Message message = new Message();//输入名字
						//message.what = 1;
						//handler.sendMessage(message);
						toMenuView();
					}
